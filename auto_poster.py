...
DELETE_PHRASES = [
    ...
    r'^.*viber.*$',
    r'^.*\bwa\.me\b.*$',
    # MAX / МАХ — мессенджер. Удаляем ТОЛЬКО короткие контактные строки
    # (МАХ + имя), а не строки комплектации, где MAX может быть частью
    # названия опции (например "система MAX Sound"). Признак контакта:
    # строка короткая (до ~30 симв) и не содержит запятых-перечислений.
    r'^\s*(?:МАХ|MAX|Мах)\s+[А-ЯA-Zа-яa-z]+\s*$',
    r'^.*max\.ru.*$',
    r'^\s*(?:Телеграм|Telegram|Телеграмм)\s*:?\s*[@А-ЯA-Zа-яa-z0-9_]*\s*$',
    r'^\s*(?:WhatsApp|Whatsapp|Ватсап|Вотсап)\s+[А-ЯA-Zа-яa-z]+\s*$',
    r'^.*[Рр]ассрочк[аеу].*$',
    r'^.*[Тт]rade.?[Іи]n.*$',
    r'^.*[Тт]рейд.?[Ии]н.*$',
